...
#' * `year()` extract the numeric `year` corresponding to `Dates`.
#' * `month()` extract the numeric `month` corresponding to `Dates`.
#' * `day()` extract the numeric `day` corresponding to `Dates`.
#'
#'
...
formatDate <- function(x, format = "dmY", sep = "/", century = NULL)
{
  if (is.character(x)) {
    ...
    stop("x must be a character or numeric.", call. = FALSE)
  }
  return(x)
}
